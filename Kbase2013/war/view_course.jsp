...
<div class="span12">
 ...
 <div class="widget widget-nopad">
		...
		try
		{
			Query q = pm.newQuery("SELECT FROM com.kbase.app.server.Course WHERE uty_id == "+ session.getAttribute("uty_id"));
			@SuppressWarnings("unchecked")
			List<Course> result = (List<Course>) q.execute();
			if(!result.isEmpty())
			{
				for(com.kbase.app.server.Course u : result)
				{
				
						String course_name = (String) u.getName();
						Long dept_id = u.getDept();
						Long id = u.getId();
						PersistenceManager pm_dept = PMF.get().getPersistenceManager();
						try
						{	
							Department d = pm_dept.getObjectById(Department.class,dept_id);
							String dep_name = d.getName();
					%>
							<tr>
								<td><% out.println(course_name);%></td>
								<td><% out.println(id.toString());%></td>
								<td><% out.println(dep_name);%></td>					
							</tr>
